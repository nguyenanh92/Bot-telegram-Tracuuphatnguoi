...
TELEGRAM_BOT_TOKEN = "8171760377:AAGnVellvwdHst5mZ-aLz8WW4uYYnaHrVIY"
...
async def dangky(update: Update, context: CallbackContext) -> None:
    ...
    await update.effective_message.reply_text("🚗 Vui lòng nhập biển số xe bạn muốn đăng ký:")
...
async def handle_dangky(update: Update, context: CallbackContext) -> None:
    ...
    if not plate_number.isalnum() or len(plate_number) < 6:
        await update.message.reply_text("⚠️ Biển số không hợp lệ.")
        return
...
async def danhsach(update: Update, context: CallbackContext) -> None:
    ...
    if not plates:
        await update.effective_message.reply_text("❌ Bạn chưa đăng ký biển số nào.")
        return
    keyboard = [[InlineKeyboardButton(f"❌ Xóa - [{plate}]", callback_data=f"remove_{plate}")]
                for plate in plates]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.effective_message.reply_text("📋 Các biển số bạn đã đăng ký:", reply_markup=reply_markup)
